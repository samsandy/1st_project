...
public class MainUI {
	...
	JPanel menu_panel, content_panel, bottom_panel, my_info_panel;
	JButton btn_login, btn_join;
	JTextField id_tf;
	JPasswordField pw_tf;
	String[] menulist = {"메인으로","캐릭터생성","채팅","게시판","마이페이지","종료"};
...
//		firstView();
	...
	public void secondView() {
		...
		menu_panel = new JPanel(new GridLayout(1,6));
		...
		for(String name : menulist) {
			...
			System.out.println(buttonlist.get(i).getName());
	...
	public void switch_panel(int menu) {
		...
		switch(menu) {
			case LOGIN :
				...
				break;
			case GOMAIN :
				gomain_panel.removeAll();
				gomain_panel.setVisible(true);
				break;
			case CREATE :
